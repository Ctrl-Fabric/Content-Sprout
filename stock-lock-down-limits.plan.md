...
overview: Encrypt imported stock assets so they are only usable inside Content-Sprout, block plaintext export/download paths for those assets, enforce a default 20-per-day stock download quota, and encrypt the quota usage index so history/count cannot be casually tampered with.
todos:
  - id: quota
  ...
  - id: quota-index-crypto
    ...
    status: completed
  - id: tests
    content: Tests for quota, crypto round-trip, export blocks, lock inheritance, and encrypted usage index
    status: completed
---
